...
describe('SUT: PassengersComponent', () => {
  ...
  it('should be not set required error to passenger controller when passenger is empty', () => {
    // arrange
    sut = sutBuilder.with_data_for_form({ Adult: null, Child: null, Infant: null } as any).build();
  ...
  it('should be not set required error to passengers controller when passengers has proper value', () => {
    // arrange
    sut = sutBuilder.with_data_for_form({ Adult: 1, Child: 1, Infant: 1 } as any).build();
  ...
  it(`should set max error on Infant when is greater than Adult in simple way`, () => {
    // arrange
    sut = sutBuilder.with_data_for_form({ Adult: null, Child: null, Infant: 1 } as any).build();

    // action
    sut = sutBuilder.with_data_for_form({ Adult: null, Child: null, Infant: 3 } as any).build();
  ...
  it(`should set max error on Infant when is greater than Adult in advance way`, () => {
    // arrange
    sut = sutBuilder.with_data_for_form({ Adult: 1, Child: null, Infant: 3 } as any).build();

    // act
    sut = sutBuilder.with_data_for_form({ Adult: 1, Child: null, Infant: 2 } as any).build();
    // assert
    expect(sut.form.controls.Infant?.hasError('max')).toBeTrue();
  ...
  it(`should not set max error on Infant when is equal or less than Adult`, () => {
    ...
    // act
    sut = sutBuilder.with_data_for_form({ Adult: 3, Child: null, Infant: 3 } as any).build();
